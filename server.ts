...
app.get("/get_product_evaluation", async (req, res) => {
  try {
    const evaluation = await db.query_product_evaluation(
...
app.get("/get_specific_order", async (req, res) => {
  const order: any = await db.query_specific_order(
...
app.get("/get_specific_product", async (req, res) => {
  const [product]: any = await db.query_specific_product(
    (req.query as { id: string }).id,
  );
  res.json(product);
});
...
app.post("/login", async (req, res) => {
  const customer_result = (await db.check_user(
    req.body.username,
    req.body.password,
  )) as loginData[];
  const seller_result = (await db.check_seller(
    req.body.username,
    req.body.password,
  )) as loginData[];
  if (customer_result.length == 0 && seller_result.length == 0)
    res.sendStatus(404);
...
app.get("/getCustomerProduct", async (req, res) => {
  try {
    const product = await db.customer_get_product();

    for (let i = 0; i < product.length; i++) {
      if (product[i].Picture == "")
        product[i].img = await getImageFromS3({ key: "default.jpg" });
      else if (
        [".jpeg", ".jpg", ".png", ".gif"].some((extension) =>
          product[i].Picture.includes(extension),
        )
      ) {
        const suffixIndex = product[i].Picture.lastIndexOf(".");
        product[i].img = await getImageFromS3({
          key: product[i].Picture.substring(0, suffixIndex),
        });
      } else {
        product[i].img = await getImageFromS3({ key: product[i].Picture });
      }
    }
...
app.post("/getComments", async (req, res) => {
  try {
    ...
    console.log("PID: ", PID);
    const comments = await db.customer_get_comments(PID);
    res.json(comments);
  } catch (err) {
    console.log(err);
  }
